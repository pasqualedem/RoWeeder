...
def torch_get_outer_edges(a, range):
    """
    Determine the outer bin edges to use, from either the data or the range
    argument
    """
...
def torch_histogramdd(thetas, rhos, rho_values, displacements=None, range=None):
    """
    ...
    for i in _range(D):
        if bins[i].ndim == 0:
            if bins[i] < 1:
                raise ValueError(
                    '`bins[{}]` must be positive, when an integer'.format(i))
            smin, smax = torch_get_outer_edges(sample[:,i], range[i])
            edges[i] = np.linspace(smin, smax, bins[i] + 1)
        elif bins[i].ndim == 1:
